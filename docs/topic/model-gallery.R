...
library(ggplot2)
library(dplyr)
opts_chunk$set(comment='.', fig.align="center",fig.path="img/model-gallery-")
...
##' 
data <- expand.ev(amt=1000, ALG=seq(0,12,4),FORM=c(1,2)) %>% mutate(F1 = ifelse(FORM==1,1,0.5))
...
plot(out, CP~time|factor(FORM),group=ALG, scales="same", auto.key=list(columns=4))
...
#+
set.seed(3203)
out <- mod %>% init(GUT=1000) %>% obsonly %>% mrgsim(Req="CP,DV") %>% as.data.frame

#+
ggplot(data=out, aes(x=time)) + geom_point(aes(y=DV),size=3) + geom_line(aes(y=CP), col="firebrick",lwd=1)
...
data <- ev(ID=1,evid=2,AUC=15, ii=48,addl=0,amt=0) %then% ev(ID=1,evid=2,AUC=0,amt=0)
...
out <- mod %>% ev(amt=200,ss=1,ii=12,addl=10)%>%
...
##' Doses and dialysis
events <- bind_rows(dial_start,dial_end,doses) %>% arrange(time) %>% as.data.frame %>% as.ev
